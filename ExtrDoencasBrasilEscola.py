...
URL_BASE = "https://brasilescola.uol.com.br/doencas/"
...
def extrair_dados(soup: BeautifulSoup):
    try:
        div = soup.find('div', class_="ler-texto")
        texto_bruto = div.prettify()
        texto = texto_bruto.split('<br/>', 1)[0] 
        texto2 = BeautifulSoup(texto, 'html.parser').text.replace('\n','').replace('\r','').replace('\t','') 
        div_titulo = soup.find('div', class_="col-xl-12 half-padding")
        titulo = div_titulo.find('h1').text.replace('\n', '')
        return texto2, titulo
    except Exception as error:
...
def criar_csv():
    with open('dados3.csv', 'a', newline='', encoding='utf-8') as file:
...
def salvar_dados_csv(titulo, texto, palavra, url):
    with open('dados3.csv', 'a', newline='', encoding='utf-8') as file:
        writer = csv.writer(file, delimiter=';')
        writer.writerow([palavra, texto, "Acesso em maio de 2024", "Site Mundo Educação", url, titulo])
