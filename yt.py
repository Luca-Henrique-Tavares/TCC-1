...
def read_sus2():
    try:
        ...
        print(e)
    try:
        df_yt = pd.read_sql_table('youtube_sus',con = connection())
    except Exception as e:
        print(e)
    try:
        df_yt
        ...
        i = 0
    while i < len(df_sus): # OBS: Caso seja menor que o tamanho do dataframe, ele irá parar de pesquisar. Obs, este i é o último id do dataframe do youtube, portanto isso inibe repetições o que é totalemente necessário para não haver repetições em uma api limitada ao uso
        id = df_sus['id'].iloc[i]
        titulo = df_sus['titulo'].iloc[i]
...
def pandas(url,titulo,canal,id):
    try: 
        if os.path.exists('youtube_sus.csv'):
            df = pd.read_csv('youtube_sus.csv')
        else:
    ...
    df.to_csv('youtube_sus.csv', index=False)#cria o arquivo csv, passando o dataframe para csv e criando o arquivo
    inserir(df)

#Função que insere os dados no banco de dados
def inserir(df):
    try:
        df.to_sql('youtube_sus',con=connection(),if_exists='replace',index=False)
    except Exception as e:
        print(e)
...
'''
ALTER TABLE `teste_tcc`.`youtube_sus` 
