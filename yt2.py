...
def connection_yt():
    try: 
        key = 'AIzaSyAPm-M1Ah4coBG9PqiqAyTyVW1bbXrRgLM'
...
def read_teste_dici():
    try:
        ...
        print(e)
    try:
        df_yt = pd.read_sql_table('youtube_dici',con = connection())
    except Exception as e:
        print(e)
    try: 
        df_yt
        i = df_yt['dici_teste_id'].iloc[-1] # pega o último id do dataframe
        ...
        i = 0
    while i < len(df_dici): # OBS: Caso seja menor que o tamanho do dataframe, ele irá parar de pesquisar. Obs, este i é o último id do dataframe do youtube, portanto isso inibe repetições o que é totalemente necessário para não haver repetições em uma api limitada ao uso
        id = df_dici['id_true'].iloc[i]
        titulo = df_dici['title'].iloc[i]
...
def pandas(url,titulo,canal,id):
    try: 
        if os.path.exists('youtube_dici.csv'):
            df = pd.read_csv('youtube_dici.csv')
        else:
            df = pd.DataFrame(columns=['titulo', 'url','nome_do_canal','data', 'fonte','dici_teste_id'])
    except Exception as e:
        print(e)
    ...
    df.to_csv('youtube_dici.csv', index=False)#cria o arquivo csv, passando o dataframe para csv e criando o arquivo
    inserir(df)

#Função que insere os dados no banco de dados
def inserir(df):
    try:
        df.to_sql('youtube_dici',con=connection(),if_exists='replace',index=False)
    except Exception as e:
        print(e)
...
'''
ALTER TABLE `teste_tcc`.`youtube_dici` 
